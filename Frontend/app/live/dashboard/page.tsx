  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--bg-primary)" }}>
      ...
      <div style={{ width: "100%", padding: "2rem" }}>
        ...
        {success && <div style={{ padding: "0.7rem 1rem", backgroundColor: "var(--accent-light)", color: "var(--accent)", borderRadius: "var(--radius-md)", marginBottom: "1rem", fontSize: "0.85rem", border: "1px solid var(--border-focus)" }}>{success}</div>}
        ...
        {isLoading ? (
          ...
          <div className="form-card" style={{ padding: "2rem", maxWidth: 600 }}>
            ...
            {!showCreate ? (
              <button onClick={() => setShowCreate(true)} className="btn-primary" style={{ padding: "0.65rem 1.5rem", fontSize: "0.9rem" }}>
                Create Stream
              </button>
            ) : (
              <form onSubmit={handleCreate} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  ...
                  <input type="text" className="input-field" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Stream title" required style={{ width: "100%", boxSizing: "border-box" }} />
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  <label style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-secondary)" }}>Description</label>
                  <textarea className="input-field" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What will you be streaming?" rows={3} style={{ width: "100%", boxSizing: "border-box", resize: "vertical" }} />
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  <label style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-secondary)" }}>Category</label>
                  <select className="input-field" value={category} onChange={(e) => setCategory(e.target.value)} style={{ width: "100%", boxSizing: "border-box" }}>
                    {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  <label style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-secondary)" }}>Tags (comma-separated)</label>
                  <input type="text" className="input-field" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="gaming, fun, live" style={{ width: "100%", boxSizing: "border-box" }} />
                </div>
                <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                  <button type="button" onClick={() => setShowCreate(false)} style={{ padding: "0.65rem 1.5rem", borderRadius: "var(--radius-md)", fontSize: "0.9rem", fontWeight: 600, backgroundColor: "var(--bg-elevated)", color: "var(--text-secondary)", border: "1px solid var(--border-light)", cursor: "pointer" }}>
                    Cancel
                  </button>
                  <button type="submit" disabled={creating} className="btn-primary" style={{ padding: "0.65rem 1.5rem", fontSize: "0.9rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
            ...
            <div className="form-card" style={{ padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", marginTop: "1.25rem" }}>
                {myStream.isLive ? (
                  ...
                  <button onClick={handleGoLive} disabled={actionLoading} className="btn-primary" style={{ padding: "0.65rem 1.5rem", fontSize: "0.9rem" }}>
            ...
            <div className="form-card" style={{ padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <input type="text" className="input-field" value={myStream.streamKey} readOnly style={{ flex: 1, fontFamily: "monospace", fontSize: "0.85rem" }} />
                <button onClick={() => { navigator.clipboard.writeText(myStream.streamKey); setSuccess("Stream key copied!"); setTimeout(() => setSuccess(""), 2000); }} style={{ padding: "0.55rem 1rem", borderRadius: "var(--radius-md)", fontSize: "0.85rem", fontWeight: 600, backgroundColor: "var(--bg-elevated)", color: "var(--text-secondary)", border: "1px solid var(--border-light)", cursor: "pointer", whiteSpace: "nowrap" }}>
                  Copy
                </button>
              </div>
              <div style={{ marginTop: "1rem", padding: "0.75rem", borderRadius: "var(--radius-md)", backgroundColor: "var(--bg-secondary)", border: "1px solid var(--border-light)" }}>
