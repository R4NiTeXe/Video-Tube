...
export default function SearchPage() {
  ...
  const { data: videosResp, isLoading: videosLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/videos?query=${encodeURIComponent(debouncedQuery)}&limit=30`,
      );
      return res.data;
    },
  ...
  const { data: channelsResp, isLoading: channelsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/videos/search/channels?query=${encodeURIComponent(debouncedQuery)}`,
      );
      return res.data;
    },
  ...
  const { data: usersResp, isLoading: usersLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/users/search?query=${encodeURIComponent(debouncedQuery)}`,
      );
      return res.data;
    },
  ...
  const videos: VideoResult[] = Array.isArray(videosResp?.data?.docs)
    ? videosResp.data.docs
    : [];
  const channels: ChannelResult[] = Array.isArray(channelsResp?.data)
    ? channelsResp.data
    : [];
  const users: UserResult[] = Array.isArray(usersResp?.data)
    ? usersResp.data
    : [];
  ...
  return (
    <>
      <PageMeta
        ...
        description={
          debouncedQuery
            ? `Search results for "${debouncedQuery}" on VideoTube.`
            : "Search for videos, channels, and users on VideoTube."
        }
        noIndex
      />
      <div style={{ minHeight: "100vh", backgroundColor: "var(--bg-primary)" }}>
        ...
        <div
          style={{ maxWidth: 1400, margin: "0 auto", padding: "1.5rem 2rem" }}
        >
          {showSuggestions && (
            <div>
              ...
              <div style={{ textAlign: "center", padding: "3rem 1rem" }}>
                ...
                <p style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>
                  Find videos, channels, and users
                </p>
              </div>
            </div>
          )}
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    style={{
                      padding: "0.5rem 1rem",
                      background:
                        activeTab === tab.key ? "var(--accent-subtle)" : "none",
                      color:
                        activeTab === tab.key
                          ? "var(--accent)"
                          : "var(--text-secondary)",
                            ...
                            >
                              ...
                              >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  >
                                    @{ch.username}
                                    {typeof ch.subscribersCount === "number"
                                      ? ` · ${formatViews(ch.subscribersCount)} subscribers`
                                      : ""}
                                  </p>
                                </div>
                              </Link>
