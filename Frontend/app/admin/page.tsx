...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"stats" | "users" | "reports" | "activity">(
    "stats",
  );
  ...
  const { data: usersRes } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/admin/users?page=${userPage}&limit=15${userQuery ? `&query=${encodeURIComponent(userQuery)}` : ""}`,
      );
  ...
  const deleteVideoMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setActionMsg("Video deleted");
      setTimeout(() => setActionMsg(""), 3000);
    },
  });

  const users = usersRes as
    { docs: AdminUser[]; totalDocs: number } | undefined;
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem" }}>
      <PageMeta
        ...
        description="VideoTube administration panel."
        noIndex
      />
      <h1
        style={{
        ...
        <div>
              ...
              >
                ...
                <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
                  {u.role === "user" ? (
                    <button
                      onClick={() =>
                        roleMutation.mutate({ userId: u._id, role: "admin" })
                      }
                      style={{
                        fontSize: "0.75rem",
                    ...
                    <button
                      onClick={() =>
                        roleMutation.mutate({ userId: u._id, role: "user" })
                      }
                      style={{
                        fontSize: "0.75rem",
                        padding: "0.35rem 0.7rem",
                  ...
                  <button
                    onClick={() => {
                      if (
                        confirm(
                          `Ban ${u.fullName}? This will delete all their content.`,
                        )
                      )
          ...
          {users && users.totalDocs > 15 && (
            ...
            >
              ...
              <button
                disabled={!users || userPage * 15 >= users.totalDocs}
                onClick={() => setUserPage((p) => p + 1)}
                style={{
                  padding: "0.4rem 1rem",
                  borderRadius: "var(--radius-md)",
                  ...
                  border: "1px solid var(--border)",
                  cursor:
                    !users || userPage * 15 >= users.totalDocs
                      ? "not-allowed"
                      : "pointer",
                }}
              >
      ...
      {tab === "activity" && (
        <div className="responsive-grid-2" style={{ gap: "1.5rem" }}>
          ...
          <div>
            ...
            >
              {(Array.isArray(activity?.recentVideos)
                ? activity.recentVideos
                : []
              ).map((v) => (
                <div
                  key={v._id}
                  ...
                  <button
                    onClick={() => {
                      if (confirm("Delete this video?"))
                        deleteVideoMutation.mutate(v._id);
                    }}
                    style={{
                      marginLeft: "auto",
                      fontSize: "0.7rem",
