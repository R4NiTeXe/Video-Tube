...
const SkeletonVideoPage = () => (
  ...
  >
    ...
    >
      ...
      <div
        className="skeleton"
        style={{ width: "100%", height: 80, borderRadius: "var(--radius-md)" }}
      />
    </div>
    <div
      style={{
        ...
        >
          <button
            ...
            style={{
              ...
              color: comment.isLiked ? "red" : "var(--text-muted)",
  ...
  useEffect(() => {
    ...
    if (video.hlsUrl && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        setAvailableQualities(Array.from(new Set(levels.map((l) => l.name))));
        const autoLevel = hls.autoLevelEnabled;
        setVideoQuality(
          autoLevel ? "auto" : levels[levels.length - 1]?.name || "auto",
        );
      });
      ...
      hls.on(
        Hls.Events.ERROR,
        (_event: unknown, data: { type: string; fatal: boolean }) => {
          if (data.fatal && !hlsDestroyed) {
            hlsDestroyed = true;
            hls.destroy();
            hlsRef.current = null;
                  ...
                  >
                    ...
                    >
                      ...
                      {rv.duration ? (
                        ...
                        >
                          {Math.floor(rv.duration / 60)}:
                          {String(Math.floor(rv.duration % 60)).padStart(
                            2,
                            "0",
                          )}
                        </div>
                      ...
                      >
                        <span
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            router.push(`/channel/${rv.owner?.username}`);
                          }}
                        >
