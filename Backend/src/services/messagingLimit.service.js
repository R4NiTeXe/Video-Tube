...
/**
 * Checks the messaging limits for both global and individual user.
...
export const checkMessagingLimit = async (identifier) => {
  ...
  if (redis && redis.status === "ready") {
    ...
    if (globalCount >= GLOBAL_DAILY_LIMIT) {
      throw new ApiError(429, "Our daily messaging capacity has been reached. Please try again tomorrow.", [{ code: "GLOBAL_EMAIL_LIMIT", resetTime: resetTimeStr }]);
    }
    if (userCount >= USER_DAILY_LIMIT) {
      throw new ApiError(429, `You have reached the daily limit of ${USER_DAILY_LIMIT} messages. Please try again tomorrow.`, [{ code: "USER_EMAIL_LIMIT", resetTime: resetTimeStr }]);
    }
  } else {
    const todayStr = new Date().toISOString().split("T")[0];
    global.messagingLimitFallback = global.messagingLimitFallback || { global: 0, users: new Map(), date: todayStr };
...
/**
 * Increments messaging limits after a successful send.
 */
