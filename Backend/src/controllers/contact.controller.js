...
const createContact = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      logger.warn("Failed to send confirmation email to user:", { error: userEmailErr.message, email });
    }

    await contact.save({ validateBeforeSave: false });
  } catch (emailErr) {
    logger.error("Failed to send contact notification email:", { error: emailErr.message });
    throw new ApiError(500, "Your message was saved but we could not send the notification. Please try again later.");
  }

  return res.status(201).json(
    new ApiResponse(201, { id: contact._id }, "Your message has been sent successfully. We will get back to you as soon as possible.")
  );
});
