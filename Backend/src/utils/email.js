...
import {
  checkMessagingLimit,
  incrementMessagingLimit,
} from "../services/messagingLimit.service.js";
...
const maskEmail = (e) => {
  const at = e.indexOf("@");
  return at > 0 ? `${e.slice(0, 2)}***${e.slice(at)}` : `${e.slice(0, 2)}***`;
};
...
const sendEmail = async ({ to, subject, html }) => {
  ...
  try {
    ...
    if (limitError?.statusCode === 429 || limitError?.status === 429)
      throw limitError;
    // For unexpected errors (Redis down, etc), log and continue
    logger.warn("Messaging limit check failed (skipping):", {
  ...
  if (isBrevoConfigured) {
    try {
      ...
      logger.debug(`[sendEmail] Brevo response status: ${response.status}`, {
        responseData,
      });
  ...
  logger.warn(
    "[sendEmail] No email provider configured — email NOT sent, logged to console"
  );
